...
function gerarLoginPadrao(
  nome: string,
  usuariosExistentes: Usuario[] = [],
  idIgnorado?: number
) {
  const partes = normalizarPartesNome(nome)
  ...
  const candidatos = [
    ...
    ...partes
      .slice(1, -1)
      .reverse()
      .filter((parte) => !particulas.has(parte))
      .map((parte) => `${partes[0]}.${parte}`),
  ]

  const loginOcupado = (login: string) =>
    usuariosExistentes.some(
      (usuario) =>
        usuario.id !== idIgnorado &&
        usuario.login.trim().toLowerCase() === login.toLowerCase()
    )

  const candidatoDisponivel = candidatos.find(
    (candidato) => !loginOcupado(candidato)
  )
  ...
  while (loginOcupado(`${base}${contador}`)) {
    contador += 1
  }
...
function carregarUsuariosPadronizados() {
  const carregados = carregarUsuarios()
  let alterou = false
  const processados: Usuario[] = []

  const normalizados = carregados.map((item) => {
    const loginPadrao = gerarLoginPadrao(
      item.nome,
      processados,
      item.id
    )
    ...
    const normalizado =
      partesNomeComSobrenome(item.nome) &&
      loginPadrao &&
      loginPadrao !== item.login
        ? {
            ...item,
            login: loginPadrao,
          }
    ...
    if (normalizado.login !== item.login) {
      alterou = true
    }

    processados.push(normalizado)
    return normalizado
  })
...
function partesNomeComSobrenome(nome: string) {
...
function Administracao({
  ...
  const [usuarios, setUsuarios] =
    useState<Usuario[]>(carregarUsuariosPadronizados)
  ...
  async function salvarUsuario() {
    ...
    if (!partesNomeComSobrenome(nomeUsuario) || !loginPadrao.includes('.')) {
      window.alert(
    ...
    <MenuPrincipal
      ...
      rodapeAcoes={
        <>
          <button
            type="button"
            className="btn-secundario adm-footer-voltar"
                ...
                <div className="adm-card-heading">
                  ...
                  {editUsuario && (
                    <button
                      ...
                      className="btn-secundario adm-cancel-edit"
                    >
                      Cancelar Alteração
                    </button>
                  )}
                ...
                <div className="adm-grid adm-grid-4">
                  ...
                  <label>
                    ...
                    <input
                      ...
                      title="O Login é gerado automaticamente no padrão nome.sobrenome."
                    />
                  </label>
                          ...
                          <tr key={item.id}>
                            ...
                            <td>
                              <button
                                ...
                                className="adm-table-action btn-secundario"
                              >
                                Editar
                              </button>
                              ...
                              <button
                                ...
                                className={`adm-table-action ${item.status === 'Ativo' ? 'adm-table-action-danger' : 'adm-table-action-success'}`}
                              >
                                {item.status ===
                                'Ativo'
                                  ? 'Inativar'
                          ...
                          <tr key={item.id}>
                            ...
                            <td>
                              ...
                              <button
                                ...
                                className={`adm-table-action ${item.ativo ? 'adm-table-action-danger' : 'adm-table-action-success'}`}
                              >
                                {item.ativo
                                  ? 'Inativar'
                                  : 'Ativar'}
