...
export default function Configuracoes({
  ...
  return (
    <MenuPrincipal
      ...
      rodapeAcoes={
        <>
          <button type="button" onClick={onVoltar} style={{ height: 40, padding: '0 15px', border: '1px solid #cbd5e1', borderRadius: 9, background: '#ffffff', color: '#334155', fontFamily: 'Inter, sans-serif', fontSize: 12, fontWeight: 600, cursor: 'pointer' }}>
            ← Voltar ao Dashboard
          </button>

          <div style={{ marginLeft: 'auto', display: 'flex', gap: 10 }}>
            <button type="button" onClick={restaurarPadroes} style={{ height: 40, padding: '0 15px', border: '1px solid #cbd5e1', borderRadius: 9, background: '#ffffff', color: '#334155', fontFamily: 'Inter, sans-serif', fontSize: 12, fontWeight: 600, cursor: 'pointer' }}>
              ↻ Restaurar Padrões
            </button>
            <button type="button" onClick={salvar} style={{ height: 40, padding: '0 17px', border: 0, borderRadius: 9, background: '#2563eb', color: '#ffffff', fontFamily: 'Inter, sans-serif', fontSize: 12, fontWeight: 700, cursor: 'pointer', boxShadow: '0 3px 8px rgba(37, 99, 235, .20)' }}>
    ...
    >
      <main style={{ width: '100%', minHeight: 'calc(100vh - 72px)', padding: '4px 20px 98px', boxSizing: 'border-box', background: '#f8fafc', color: '#0f172a', fontFamily: 'Inter, -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif' }}>
        <header style={{ marginBottom: 18 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 4 }}>
            <div style={{ width: 4, height: 27, borderRadius: 4, background: '#2563eb' }} />
            <h1 style={{ margin: 0, color: '#0f172a', fontSize: 25, lineHeight: 1.2, fontWeight: 750, letterSpacing: '-.45px' }}>Configurações</h1>
          </div>
          <p style={{ margin: '0 0 0 14px', color: '#64748b', fontSize: 13, lineHeight: 1.4 }}>Parâmetros gerais e preferências do sistema.</p>
        </header>

        {mensagem && (
          <div role="status" style={{ marginBottom: 14, padding: '11px 14px', display: 'flex', alignItems: 'center', gap: 9, border: '1px solid #bbf7d0', borderRadius: 10, background: '#f0fdf4', color: '#166534', fontSize: 12, fontWeight: 600 }}>
            <span style={{ width: 23, height: 23, display: 'inline-flex', alignItems: 'center', justifyContent: 'center', borderRadius: '50%', background: '#dcfce7', color: '#16a34a', fontWeight: 800 }}>✓</span>
            {mensagem}
          </div>
        )}

        {/* ADMINISTRAÇÃO */}
        <section style={{ ...cardBase, marginBottom: 14, borderColor: '#bfdbfe', background: 'linear-gradient(135deg, #ffffff 0%, #f8fbff 100%)' }}>
          ...
          <div style={{ padding: '15px 18px 15px 22px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 18 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 13, minWidth: 0 }}>
              <div style={sectionIcon('#eff6ff', '#2563eb')}>⚙</div>
              <div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
                  <h2 style={{ margin: 0, color: '#0f172a', fontSize: 15, fontWeight: 750 }}>Administração do Sistema</h2>
            ...
            <button type="button" onClick={onAdministracao} style={{ flexShrink: 0, height: 39, padding: '0 16px', border: 0, borderRadius: 9, background: '#2563eb', color: '#ffffff', fontFamily: 'Inter, sans-serif', fontSize: 12, fontWeight: 700, cursor: 'pointer', boxShadow: '0 3px 8px rgba(37, 99, 235, .18)' }}>
              ⚙ Abrir Administração
            </button>
          </div>
        </section>
        ...
        {/* IDENTIFICAÇÃO */}
        <section style={{ ...cardBase, padding: 18, marginBottom: 14 }}>
          <div style={sectionHeader}>
            <div style={sectionIcon('#eff6ff', '#2563eb')}>▣</div>
            <div>
              <h2 style={{ margin: 0, fontSize: 15, fontWeight: 750 }}>Identificação do Sistema</h2>
          ...
          <div className="config-identificacao" style={{ display: 'grid', gridTemplateColumns: 'repeat(3, minmax(0, 1fr))', gap: 12 }}>
            ...
            ].map(([label, valor, icone, fundo, cor]) => (
        ...
        <section style={{ ...cardBase, padding: 18, marginBottom: 14 }}>
          <div style={sectionHeader}>
            <div style={sectionIcon('#fff7ed', '#ea580c')}>◷</div>
            <div>
              <h2 style={{ margin: 0, fontSize: 15, fontWeight: 750 }}>Regras de SLA</h2>
        ...
        <section style={{ ...cardBase, padding: 18, marginBottom: 14 }}>
          <div style={sectionHeader}>
            <div style={sectionIcon('#fefce8', '#ca8a04')}>🔔</div>
            <div>
              <h2 style={{ margin: 0, fontSize: 15, fontWeight: 750 }}>Notificações</h2>
              <p style={{ margin: '3px 0 0', color: '#64748b', fontSize: 12 }}>Controle quais eventos devem ficar habilitados nas preferências.</p>
            </div>
            <span style={{ marginLeft: 'auto', padding: '5px 9px', borderRadius: 999, background: '#f0fdf4', color: '#15803d', fontSize: 10, fontWeight: 800 }}>
        ...
        <section style={{ ...cardBase, padding: 18, marginBottom: 14 }}>
          <div style={sectionHeader}>
            <div style={sectionIcon('#f5f3ff', '#7c3aed')}>▤</div>
            <div>
              <h2 style={{ margin: 0, fontSize: 15, fontWeight: 750 }}>Preferências de Interface</h2>
        ...
        <section style={{ ...cardBase, padding: 18 }}>
          <div style={sectionHeader}>
            <div style={sectionIcon('#ecfeff', '#0891b2')}>↔</div>
            <div>
              <h2 style={{ margin: 0, fontSize: 15, fontWeight: 750 }}>Fluxo das Demandas</h2>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
            {STATUS.map((status) => {
              ...
              return (
                <span key={status} style={{ display: 'inline-flex', alignItems: 'center', gap: 7, minHeight: 33, padding: '0 11px', border: `1px solid ${estilo.border}`, borderRadius: 999, background: estilo.bg, color: estilo.color, fontSize: 11, fontWeight: 700 }}>
